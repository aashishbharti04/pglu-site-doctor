...
def to_markdown(report: SiteReport) -> str:
    s = report.scores
    out = [
        f"# pglu-site-doctor report — {report.start_url}",
        "",
...
def to_junit(report: SiteReport) -> str:
    ...
    return (
        ...
        f'<testsuite name="pglu-site-doctor" tests="{len(cases)}" failures="{failures}">\n'
        f"  {body}\n"
        "</testsuite>\n"
    )
...
<title>pglu-site-doctor report — {e(report.start_url)}</title>
<style>
...
<body><div class="wrap">
  ...
  <footer>Generated by <a href="https://github.com/aashishbharti04/pglu-site-doctor">pglu-site-doctor</a>.</footer>
</div></body></html>
"""
